...
usage() {
  echo "$0 ORG_PROJ ARCH_FILTER NAME_FILTER"
  echo "    ORG_PROJ    - organization/projectname"
  echo "    ARCH_FILTER - optional extra filter to further limit rpm selection"
  echo "    NAME_FILTER - optional extra filter to further limit rpm selection"
...
RPM_URLS=$(cat ${API_JSON} \
  | jq \
    ...
    '.assets | sort_by(.created_at) | reverse | .[] | select(.name|test($arch_filter)) | select(.name|test($name_filter)) | select (.name|test("rpm$")) | .browser_download_url')
